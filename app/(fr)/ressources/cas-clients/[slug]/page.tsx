      ...
      <article className="pt-28 sm:pt-32 pb-16">
        <div className="container max-w-4xl">
          ...
          <div className="mt-10 space-y-10">
            ...
            <section className="bg-muted/30 border border-border/50 rounded-2xl p-6"><h2 className="text-2xl font-bold mb-4">Les livrables de la mission</h2><ul className="list-disc pl-5 space-y-3">{item.deliverables.map(text => <li key={text}>{text}</li>)}</ul>{"relatedService" in item && <p className="mt-5 leading-relaxed text-muted-foreground">Pour mettre en place un P&amp;L par canal et une revue régulière des marges, découvrez notre <Link href={item.relatedService.href} className="text-iter-violet underline">{item.relatedService.label}</Link>.</p>}</section>
